...
impl<T: BeaconChainTypes> BlockLookup<T> {
    ...
    pub fn new(chain: Arc<BeaconChain<T>>, beacon_processor_send: mpsc::Sender<BeaconWorkEvent<T>>, log: Logger) -> Self {
        BlockLookup {
        chain,
    ...
    pub async fn blocks_by_root_response(
        &mut self,
        peer_id: PeerId,
    ...
    pub fn inject_error(&mut self, peer_id: PeerId, request_id: RequestId, network_context: &mut SyncNetworkContext<T::EthSpec>) {
    ...
    async fn single_block_lookup_response(
        ...
        match block_result {
            ...
            Err(BlockError::ParentUnknown { .. }) => {
                ...
                self.add_unknown_block(peer_id, block, network_context);
            }
